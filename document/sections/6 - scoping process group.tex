...
Per definire lo scope di progetto è possibile utilizzare tantissimi strumenti, templates e processi, ce ne sono veramente tanti, anche la riunione è considerabile uno strumento, eccone alcuni:
\begin{itemize}
...
\begin{itemize}
	...
	\item completamento di alcuni componenti entro una certa data;
...
Lo scopo è raccogliere tutti i requisiti (Requirements Gathering), partendo da requisiti di alto livello fino ad arrivare a quelli di basso livello, per cui sarà necessario sempre di più essere a contatto con lo stakeholder. Gli strumenti a disposizione per evitare problemi durante la raccolta dei requisiti sono i seguenti:
\begin{itemize}
	...
	\begin{info}[]
		...
		\begin{itemize}
			...
			\item L'approccio funziona bene per i processi inter-funzionali.
		\end{itemize}
		$\;\;$\textbf{Rischi:}
		\begin{itemize}
	...
	\begin{info}[]
		...
		\begin{itemize}
			...
			\item La formazione per utilizzarli è costosa.
		\end{itemize}
	\end{info}
\end{itemize}
...
Una buona pratica è mantenere le user story abbastanza separate ed indipendenti tra loro, in modo da poterle distribuire meglio sugli sprint e bilanciare anche meglio i rischi. In questo caso il problema si sposta più sulle dipendenze.
...
Quando si scrivono user story bisogna stare attenti a scriverle in modo che siano "interessanti". Importante è applicare dei criteri, valgono tutti i criteri che valgono anche in RBS. In più si possono utilizzare altri approcci, ad esempio l'approccio INVEST:
\begin{itemize}
	...
	\item \textbf{Small}: abbastanza piccole per essere completate in un'iterazione. Nel caso in cui non lo siano abbastanza e non è possibile dividerle ulteriormente resta sempre possibile aumentare la durata dello sprint;
...
\noindent Importante da definire per l'assegnazione dei ruoli. L'identificazione dell'utente è importante per definire quali azioni sono possibili per ogni classe di utenti. Il team NON è mai il protagonista delle User Story. I protagonisti delle user story sono sempre gli user. Anche in questo caso esiste un dibattito su questo punto. L'importante è chiarire come usare gli strumenti e non utilizzarli in maniera non ben definita.
\paragraph{Actions}
...
\paragraph{Benefits}
Non necessariamente una user story corrisponde ad un solo beneficio. Magari ci sono pochi benefici condivisi da più user story, o poche user story che condividono più benefici comuni. Il beneficio non è necessariamente immediato per l'utente, può esserlo anche per altri stakeholders.
...
\textbf{CoS}: vuoto (è un parametro opzionale, ma la mancanza di una CoS potrebbe indicare la mancanza di business value, secondo l'autore questo parametro si può omettere, ma è questione di opinioni).\newline
...
\begin{warn}
	...
	Come al solito è questione di opinioni, dipende anche dalla user story. Non c'è una vera e propria regola, a volte è più facile definire prima la user story per poi tirare fuori la CoS, mentre a volte è proprio la CoS che ci aiuta nella definizione della user story.
\end{warn}
...
Un processo aziendale è un insieme di attività con dettaglio precisato che prendono dell'input (può essere per esempio un ordine o un materiale, può essere di varia natura, concreto o astratto) da più sorgenti e producono un cambiamento di stato che fornisce business value.
...
\noindent Diverse rappresentazioni possono donarci diverse informazioni, il tipo di rappresentazione dipende fortemente dal tipo di processo.
...
\begin{itemize}
	\item \textbf{Statement of work (SOW)}: è la descrizione del risultato che vogliamo ottenere. Non è ben specificato in che modo va scritto, l'importante è che le idee siano chiare per ogni parte.
